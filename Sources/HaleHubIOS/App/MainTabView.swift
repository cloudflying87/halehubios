...
struct MainTabView: View {
    ...
    /// Tabs built dynamically from the user's permissions. Order is fixed;
    /// tabs simply don't appear when the user lacks access. Finance lives
    /// inside Account (rarely accessed, adults only) so we never exceed 5
    /// visible tabs for any permission set.
    ///
    /// Tab order: Meals → Shopping → Totes → Vehicles → Account
    ...
    private func handleURL(_ url: URL) {
        ...
        case "totes":
            if let idx = tabIndex(for: .totes) { selectedTab = idx }
        default:
            break
        }
    }
...
enum TabSpec: Hashable {
    case meals, shopping, totes, vehicles, account

    var title: String {
        switch self {
        case .meals:    return "Meals"
        case .shopping: return "Shopping"
        case .totes:    return "Storage"
    ...
    var icon: String {
        ...
        case .totes:    return "shippingbox.fill"
